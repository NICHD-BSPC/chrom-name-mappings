...
if __name__ == '__main__':
    ...
    ap.add_argument('--out_b_not_a', help='''Output file b_not_a to create, default is %(default)s''',
                    default='b_not_a')
    ap.add_argument('--out_filtered', help='''Output file for filtered-out chromosomes to create, default is %(default)s''',
                    default='filtered_out')
    args = ap.parse_args()
    ...
    for k in f1_d.keys():
        if k in f2_d:
            print('{0}\t{1}'.format(f1_d[k], f2_d[k]), file=out)
    ...
    with open(args.out_filtered, 'w') as fout:
        if out_f1_d:
